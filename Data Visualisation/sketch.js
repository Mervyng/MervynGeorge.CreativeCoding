...
function setup() {
  ...
  numbers.reverse();  // Reverse the array once

  let maxNumber = max(numbers);  // Calculate the maximum number once
  let w = width / numbers.length;  // Calculate the width of each rectangle

  for (let i = 0; i < numbers.length; i++) {
    let n = numbers[i];
    let x = map(i, 0, numbers.length, 0, width);
    let h = map(n, 0, maxNumber, 0, height);
    let y = height - h;
    let c = map(n, 0, maxNumber, 0, 360);
    fill(c, 100, 100);
    rect(x, y, w, h);
  }
}
...
function windowResized() {
  ...
  setup();  // Re-run setup to redraw the visualization on window resize
}
